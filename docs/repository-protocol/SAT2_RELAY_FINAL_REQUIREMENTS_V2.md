...
> 本文件是 Relay 2.0 的历史最终需求记录，保留用于追溯，不是当前运行合同。当前规则以 [`../SAT2_CHAT_RELAY_PROTOCOL.md`](../SAT2_CHAT_RELAY_PROTOCOL.md)、[`../RELAY_2.2_OPERATION.md`](../RELAY_2.2_OPERATION.md) 和仓库根目录的 `sat2AI协作方式.md` 为准。
>
...
- 通过清晰 UI 显示当前任务、队列、故障阶段和恢复动作；新的未解决警报通过浏览器通知主动提示。
...
- 浏览器重启后自动恢复绑定；
- 只允许绑定含 `/c/<conversation-id>` 的具体 Session；项目首页或新建聊天入口必须拒绝；
- 默认拒绝将同一 Session 绑定到多个角色；仅在设置页显式开启“开发模式”时允许，Deep Doctor 仍报告该风险；
...
- 深度 doctor 结果；
- 一键导出的脱敏诊断包：配置摘要、doctor、任务、队列、评论 outcome、警报、heartbeat 和最近 250 行结构化日志；
- 本地 JSONL 日志必须自动轮转，默认最多 5 个 10 MiB 备份，防止长期运行无限增长。
...
11. 完整 v2 `authorize → ACK → checkpoint → Mentor accepted` 状态链测试；
12. 一键诊断包不得包含 GitHub PAT、本地 API token 或完整投递正文。
